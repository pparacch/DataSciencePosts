...
ggplot(mpg, aes(displ, fill = drv)) + 
  stat_densityCommon(bandwidth = 1, geom = "area", position = "stack", alpha = 0.4)
```
...
GeomCentroidPolygon <- ggproto("GeomCentroidPolygon", Geom,
                               ...
                               draw_group = function(data, panel_scales,coord){
                                 ...
                                 coords <- coord$transform(data, panel_scales)
                                 first_row_col <- coords[1, "colour"]
                                 
                                 centroid_x <- mean(coords$x, na.rm = T)
                                 centroid_y <- mean(coords$y, na.rm = T)
                                 #print(paste(n, centroid_x, centroid_y, 
                                 #           first_row_col, sep = ", "))
                                 
                                 grid::pointsGrob(x = centroid_x, y = centroid_y,
                                                  gp = grid::gpar(col = first_row_col),
                                                  pch = 4)
                                 
                               })

geom_centroid_polygon <- function(mapping = NULL, data = NULL, stat = "identity",
                                position = "identity", na.rm = FALSE, show.legend = NA, 
                                inherit.aes = TRUE, ...) {
  layer(
