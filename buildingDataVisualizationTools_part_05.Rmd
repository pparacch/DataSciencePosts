...
Possible __grobs__ that can be created include circles, lines, points, rectangles, polygons, ... Once a __grob__ is created, it can be modified (using the `editGrob` function) and then drawn (using the `grid.draw` function).

```{r circleExample1, collapse = T}
...
```{r exampleGrobObjects, collapse = T}
...
```{r exampleGrobGraphicalOutput, collapse = T}
